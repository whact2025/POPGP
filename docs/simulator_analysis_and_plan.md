...
**Version:** 2.1  
**Date:** February 2026  
**Status:** Historical planning snapshot; current scientific status is tracked in `docs/scientific_hardening/`
...
The finite variational search is implemented, but the proposed causal gradient flow is not. `Simulator.run_pi_res()` enumerates equal-size partitions, applies the implemented admissibility checks, and minimizes the sampled leakage objective with drift as a tie-breaker. Exhaustive minimization does not establish that the same partition is a stable local-flow attractor. In the selected 8-qubit Heisenberg-chain benchmark, contiguous two-qubit blocks are the unique enumerated minimizer.
...
3. **Local metric `h_ab(x_i)`:** the implemented prototype fits neighbor distances
   in MDS coordinates with a ridge penalty toward the coordinate identity:
   ```
   min_{h ≻ 0} Σ_{j∈N(i)} (d_G(i,j)² − Δx^T h Δx)² + λ_spd ‖h − I‖_F²
   ```
4. **Metric-fit identifiability:** condition number `κ(M)` of the design matrix. A
   large value is not an event horizon or spacetime singularity.
...
| Local metric h_ab | ✅ Embedding-coordinate prototype | Grid example | Not planned |
| SPD regularization | ✅ Ridge plus eigenvalue floor | Unit tests | Not planned |
| Metric-fit condition diagnostic | ✅ | Grid example | Not planned |
| Delaunay triangulation | ✅ Declared embedding proxy | Grid example | Planned (CGAL) |
| Regge deficit angles | ✅ 2D proxy only | Grid example | Planned |
| Closure mismatch M(L) | ❌ | ❌ | Planned |

#### Gap Assessment: **MODERATE** (downgraded from CRITICAL)

Steps 1–2 (finite spectral-dimension diagnostic and MDS embedding) are implemented. Selected chain and grid benchmarks return D*=1 and D*=2, respectively, but the Hamiltonians encode those interaction graphs and the penalty sensitivity/non-geometric controls prevent interpreting this as continuum dimension emergence.
...
1. **Clock-rate potential Φ:** Gauge-fixed solution to `(Δ_w + μ²I)Φ = δρ` on the weighted graph.
2. **Source term:** An experimentally classified local contrast; no candidate is yet an established stress-energy source.
3. **Reference state:** Explicit KMS thermal state for the finite-system source experiments.
4. **Screening mass:** `μ` is a numerical screening parameter; neither `μ = 0` nor `μ > 0` establishes a continuum `1/r` law.
5. **Zero-mode handling:** Enforce source compatibility and a mean-zero potential when `μ = 0`.
...
| Araki relative entropy | ✅ Implemented; available as an explicitly experimental source | Raw candidate fails the linear-response test | ❌ |
| KMS reference state | ✅ Caller-supplied reference supported | Constructed in the exact many-body source-law experiment | ❌ |
| Sparse solve for Φ | ✅ `torch.linalg.solve` on the screened or augmented mean-zero system | Source-law and gravity-well examples | `clock.cu` — still identity map |
| Zero-mode handling | ✅ Subtract/validate source mean and solve for mean-zero Φ when μ=0 | Covered by unit and scientific tests | ❌ |
| Proper time dτ | ✅ `dτ = β_0 · exp(Φ) · dS_act` | Computed | ❌ |
| Line element | ❌ (requires local metric from Phase 3) | ❌ | ❌ |

#### Gap Assessment: **MODERATE** (downgraded from CRITICAL)

The clock potential pipeline is functional end-to-end on the Python side. The `Simulator` constructs the graph Laplacian, solves the finite Poisson-like constraint with explicit zero-mode handling, and computes the configured clock-rate diagnostic. The resulting Φ profiles are numerical diagnostics; their interpretation as a physical lapse remains unvalidated.

Remaining deficiencies:
- **Source term:** von Neumann entropy remains the default placeholder. Raw relative entropy is quadratic in the tested weak perturbation, and the one-cell modular-energy candidate is blind in the symmetric KMS control. The microscopic KMS energy-density candidate passes finite exact-chain diagnostics but depends on a declared Hamiltonian decomposition and is not a validated stress-energy law.
- **Temporal averaging** of any candidate contrast is not implemented.
...
**Framework E4, §4.4.2a, F3** treats Lorentz recovery as a conjectural matching target. Verification requires:
...
4. A derived mapping from model coefficients to observable-specific current bounds.

**Status:** The finite coarse-graining benchmark checks sampled global SU(2)
equivariance: for the declared tensor-product action and partial trace this is an
analytic identity regression. It constrains rotations only. No general coarse-graining
map, boost, dispersion, RG-suppression, or experimental-bound calculation exists.
...
| **Π_time** | Graph Laplacian construction from weight matrix; screened or augmented mean-zero solve for `(Δ_w + μ²I)Φ = δρ`; clock-rate diagnostic `dτ = β_0 · exp(Φ) · dS_act`. Source options are classified as placeholder or experimental candidates. | Identify a state-independent localized source law, add temporal averaging, demonstrate refinement/scaling, and validate against an independent clock observable. |

---

### 9.1b Examples Consolidation — ✅ IMPLEMENTED

The old model scripts (`src/toy/`, `src/native/`) were replaced with reproducible finite examples that exercise the unified `Simulator` API. Their checks validate named numerical invariants only; they are not physical validation of the framework.
...
**Goal:** Test the numerical Green-function, symmetry, zero-mode, and clock-sign behavior of the finite graph constraint. This does not establish a gravitational field equation.
...
3. Injects a small negative diagnostic source $\delta\rho = -0.01$ at the center cell.
4. Solves $(\Delta_w + \mu^2 I)\Phi = \delta\rho$ with $\mu = 0.1$, subtracts the finite-graph source mean, and normalizes the potential to mean zero.
5. Groups cells by BFS graph distance from source; tests monotonic decay and lattice symmetry.
6. Checks the configured redshift sign $1 + z = \exp(\Phi_{\mathrm{boundary}} - \Phi_{\mathrm{source}})$.
...
**Key finding:** The mean-zero solve preserves the available lattice-shell symmetry and produces a negative diagnostic well. The 3×3 graph has only two noncentral shells and cannot establish a continuum falloff.

#### Acceptance Criteria — ✅ ALL MET

| Criterion | Result |
|-----------|--------|
| Phi rises monotonically away from the negative source | ✅ finite-shell diagnostic passes |
| Grid symmetry preserved (< 5% asymmetry) | ✅ 0.00% spread at all shells |
| Clock/redshift sign convention | ✅ source clock is slower and emitter-to-boundary redshift is positive |
...
- `metric_fit_condition(design_matrices) → Tensor[N]`
  Returns `κ(M_i)` at each node. High values flag poor reconstruction
  identifiability, not a physical horizon or singularity.
...
- `metric_fit_condition()` returns low κ for well-conditioned embeddings.

---

### 9.5 Phase 4 — Π_time Source-Law Falsification and Validation

**Goal:** Falsify or validate localized source candidates against an explicit KMS reference without treating any finite diagnostic as an established gravity law.
...
- ✅ Screened solve or augmented mean-zero solve for `(Δ_w + μ²I)Φ = δρ`.
- ✅ Proper time `dτ = β_0 · exp(Φ) · dS_act`.
- ✅ `araki_relative_entropy()` implemented in both backends and tested as a negative control.
- ✅ Explicit caller-supplied KMS reference supported by the candidate source models.
- ✅ Microscopic KMS energy-density candidate has nonzero susceptibility in the declared non-affine finite KMS family, passes nested-window and sign diagnostics, and has separate conservation/spreading controls. Affine-mixture unit slopes are identity regressions; an isospectral unitary family is quadratic.
- ❌ Raw relative entropy fails the first-order response criterion; the reduced-state modular candidate is blind in the symmetric control.
- ❌ Temporal averaging of contrast not implemented.
- ❌ Diagnostic potentials (Φ_t vs Φ_s) not implemented.

#### Remaining Deliverables

**4.1 Source-candidate comparison**
- Preserve raw relative entropy and reduced-state modular energy as named negative controls.
- Test whether the microscopic candidate `−β Δ⟨h_i⟩` remains local and conserved under graph refinement, alternate Hamiltonian decompositions, boundary conditions, and larger systems.
- Search for a localization prescription that is not merely inherited from a predeclared microscopic interaction graph.

**4.2 Temporal averaging**
- Evolve state over `[s_0, s_0 + Δs]`, compute each surviving source candidate at every sample, and average.
...
- Under refinement and controlled boundaries, measure rather than assume whether a localized source has any stable continuum falloff.
- Compare `Φ_t` with an independently reconstructed spatial diagnostic; agreement is a future closure test, not an acceptance assumption.
...
- `gravitational_redshift(phi_emitter, phi_observer) → float`
  Computes `z = exp(Φ_observer − Φ_emitter) − 1` under the configured clock mapping.
...
- **F3 — Lorentz violation:** Measure direction-dependent dispersion and common limiting speeds, derive the map to experimental observables, and compare with the applicable current bounds.
...
Phase 4: Strict Π_time                        MODIFICATIONS + NEW
    │                                          ────────────────────
    ├── simulator.py ─────────────────────── candidate-source comparison and averaging
...
| R4 | Graph Laplacian solve for μ = 0 has a zero mode (constant shift). | LOW | **Mitigated** | `Simulator.run_pi_time()` enforces source compatibility and solves an augmented system with mean-zero Φ; no arbitrary node is pinned. |
