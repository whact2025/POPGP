...
| Source | Temporally averaged, vacuum-relative quantity with conserved linear response | Positive von Neumann placeholder plus explicit-reference negative relative-entropy and modular-energy candidate modes | Candidates lack temporal averaging, conservation/localization validation, and covariance; raw relative entropy is quadratic and entropy-confounded at fixed energy. |
