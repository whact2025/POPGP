...
| `GATE-PORTABLE-ARTIFACT-CONTRACT` | Cross-platform regeneration | active implementation gate | `tests/unit/test_validation_artifact_contract.py::test_visual_contract_rejects_geometry_or_pixel_weakening`; `tests/unit/test_reproduction_boundary.py::test_repository_residue_allows_only_declared_generated_paths` | contract-equivalent visual and declared result drift accepted → changed geometry/pixels, source bytes, or untracked residue rejected |
| `GATE-STARTUP-SURFACE-INTEGRITY` | Locked Python startup surface | active implementation gate | `tests/unit/test_reproduction_boundary.py::test_snapshot_hash_and_post_sync_surface_are_fail_closed` | measured post-sync platform surface unchanged → injected startup hook or altered manifest rejected |
