...
\textbf{P1 (Unique physical projection).} There exists a unique, necessary projection map $\Pi$ from substrate structure to projection-level effective physics.  
...
\begin{equation}
\alpha: SU(2) \to \text{Aut}(\mathcal{A})
\end{equation}
...
\begin{equation}
E_i \circ \alpha_g = \alpha_g \circ E_i \quad \text{for all } g \in SU(2) \text{ and all } i.
\end{equation}

\textbf{The ``Custodial Symmetry'' Mechanism (Lorentz Protection).}
By forcing the local algebra of each funnel to transform as a singlet under the internal group, the most dangerous ``lattice artifact'' operators --- specifically, the tree-level dimension-4 Lorentz-violating operators that are not singlets under the induced rotation group of the tangent space --- are suppressed by the symmetry-commuting map $E_i$. With these leading artifacts eliminated, continuous Lorentz invariance becomes a highly stable \textbf{IR fixed point} of the RG flow governing the emergent effective field theory. 
...
\end{equation}
It depends only on $E_j$ for cells $j$ in the \textbf{Lieb-Robinson neighborhood} $N_{\text{LR}}(i)$ of cell $i$:
\begin{equation}
N_{\text{LR}}(i) := \{ j \in V : d_G(i,j) \leq v_{\text{LR}} \cdot \Delta s \}
\end{equation}
...
\textbf{Causality guarantee.} Because $\delta_i \mathcal{L}_{\text{leak}}$ depends only on $E_j$ for $j \in N_{\text{LR}}(i)$, updates at cells $i$ and $k$ with non-overlapping Lieb-Robinson neighborhoods \textbf{strictly commute}:
\begin{equation}
[ \partial_s E_i , \partial_s E_k ] = 0 \quad \text{whenever } N_{\text{LR}}(i) \cap N_{\text{LR}}(k) = \emptyset.
\end{equation}
...
\textbf{Variational interpretation (Thermodynamic, not Computational).} The causal gradient flow defined above is a \textit{physical relaxation process}, not an algorithmic instruction. Crucially, this relaxation requires the substrate to act as an \textbf{open thermodynamic system}: interaction-induced decoherence continually drives cell entropies toward the leakage death-threshold, so the flow can only converge to a stable fixed point if cells are capable of \textbf{exporting excess phase-flow entropy to the environment} (a mechanism analogous to radiative cooling).
...
\textbf{Weighted graph \& Graph geodesic distance.} Define a \textbf{continuous} edge weight kernel $w_{ij} := \kappa(I_{ij})$. With edge lengths $d_{ij}$, define graph-geodesic distance:
\begin{equation}
...
\end{equation}
where $\Delta_w$ is the graph Laplacian on the mutual-information weighted graph, $\delta\rho$ is the source term, and $\mu$ is a screening mass (IR regulator).
...
\begin{description}
    \item[\textbf{Structural (discrete) choices}] (Architecture decisions) Choice of algebra class for $\mathcal{A}_{\text{rel}}$; choice of internal algebra $\mathcal{A}_F$; compact internal symmetry group; uniqueness of projection postulate. \textit{Must be defended by conceptual minimality and consistency.}
...
\textbf{1. Discrete Regge Closure.} Compute the curvature using \textbf{Regge Calculus}. The deficit angle $\epsilon_h$ at hinge $h$ is:
\begin{equation}
...
\textbf{2. The Closure Mismatch Functional.}
\begin{equation}
M(L) := \left\| G_{\text{Regge}} - 8\pi G \, \Pi_{\text{graph}}(T_{\mu\nu}^{\text{eff}}) \right\|_L
\end{equation}

\textbf{3. Weak-field consistency via discrete diagnostics.} We extract an effective mass density $\rho_{\text{mass}}$ from the discrete potential using the \textbf{discrete Poisson equation}:
\begin{equation}
4\pi G \rho_i \approx (\Delta_{\text{Delaunay}} \Phi)_i
\end{equation}
...
\begin{description}
    ...
    \item[\textbf{Emergent locality} $I_{ij}, d_G$] \textit{(Definition)} Locality is defined from correlations and multi-hop routing on weighted graphs (no geometric inputs). \textbf{Failure mode:} Definition is checkable; failure mode is pathological graphs (disconnectedness, non-manifold structure) for physically relevant states.
    ...
    \item[\textbf{Spacetime metric} $g_{\mu\nu}$] \textit{(Definition)} $g$ is defined by the embedding-derived $h$ plus the clock mapping $\tau$ (no separate constitutive law $g=F(\rho)$). \textbf{Failure mode:} Pure definition; key is whether the reconstructed discrete geometry (Delaunay complex + Regge curvature) satisfies GR closure natively.
    \item[\textbf{GR closure mismatch} $M(L)$] \textit{(Definition + Matching)} The discrete Regge Einstein tensor $G_{\text{Regge}}$ on the Delaunay simplicial complex must satisfy Einstein closure within tolerance. \textbf{Failure mode:} Falsified if no small-parameter instantiation yields small $M(L)$ across standard tests.
    ...
    \item[\textbf{Local Lorentz covariance}] \textit{(Matching)} Despite a preferred phase-order foliation in the construction, local Lorentz symmetry must emerge as an excellent IR symmetry. \textbf{Failure mode:} Constrained by precision Lorentz-violation bounds; falsified if implied violations exceed limits.
\end{description}
...
CA models demonstrated \textbf{emergent spatial clustering} (``purity shields''). Disabling the cooling channel caused \textbf{total population extinction}, confirming that emergent geometry requires an open thermodynamic substrate.
...
\begin{itemize}
    \item \textbf{Volume Collapse:} If the Delaunay complex degenerates (slivers, zero-volume tetrahedra) despite a growing $V_P$, the geometry is not manifold-like.
