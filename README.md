...
<p align="center">
  <img src="examples/physics_qg/grid_2d/results/embedding.png" width="48%" alt="2D MDS embedding recovered from MI graph"/>
  &nbsp;
  <img src="examples/physics_qg/gravity_well/results/gravity_well.png" width="48%" alt="Newtonian gravity well from screened Poisson solve"/>
</p>

<p align="center"><em>
<b>Left:</b> 2D grid topology recovered via MDS on graph-geodesic distances derived from mutual information of a 9-qubit Heisenberg thermal state. The algorithm receives no spatial input; it recovers the exact lattice connectivity and dynamically selects D* = 2.
<b>Right:</b> Clock-rate potential &Phi; from a localized source on a 3&times;3 MI-weighted correlation graph. The screened graph-Poisson equation (&Delta;<sub>w</sub>&nbsp;+&nbsp;&mu;&sup2;I)&Phi;&nbsp;=&nbsp;&delta;&rho; produces monotonic radial falloff, preserves exact lattice symmetry, and yields discrete gravitational redshift 1&nbsp;+&nbsp;z&nbsp;=&nbsp;exp(&Phi;<sub>A</sub>&nbsp;&minus;&nbsp;&Phi;<sub>B</sub>). All edge weights are derived from quantum mutual information.
</em></p>

> **To regenerate these figures:** `uv run python -m examples.physics_qg.grid_2d && uv run python -m examples.physics_qg.gravity_well`
...
# Additional examples
uv run python -m examples.physics_qg.chain_1d   # 1D geometry + stability selection
uv run python -m examples.physics_qg.ca_model   # Cellular automata radiative cooling
```
...
**Run:** `uv run python -m examples.physics_qg.chain_1d`
...
**Run:** `uv run python -m examples.physics_qg.grid_2d`
...
**Run:** `uv run python -m examples.physics_qg.grid_2d` (dimension selection) / `uv run python -m examples.physics_qg.chain_1d` (1D validation)
...
**Computed output:** Localized point source on a $3 \times 3$ MI-weighted correlation graph produces monotonic radial $\Phi$ falloff, exact lattice symmetry preservation (0% asymmetry), and discrete gravitational redshift $1 + z = \exp(\Phi_A - \Phi_B)$.
**Run:** `uv run python -m examples.physics_qg.gravity_well`
...
*   `examples/`: Runnable demonstrations (each is a Python package with its own README).
    *   `physics_qg/`: Quantum gravity examples.
